...
Both examples are self-contained: each deploys an `nginx:alpine` pod,
runs a port-forward through `spdy-mux`, and deletes the pod. Only
requirement is a reachable cluster via `KUBECONFIG` (or `~/.kube/config`).
...
- `k8s_raw_spdy` - legacy `Upgrade: SPDY/3.1` over HTTP/1.1, the original
  kubectl wire format. Uses `split_raw_spdy`.
- `k8s_websocket_over_spdy` - modern WebSocket-tunnelled SPDY (KEP-4006,
  `SPDY/3.1+portforward.k8s.io`, default since Kubernetes 1.31). Uses
  `split_fastws`.

For pool management, pod watching, recovery callbacks, graceful drain,
and automatic fallback between the two paths, use the higher-level
`kube-portforward` crate which sits on top of this one.
